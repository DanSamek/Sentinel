...
namespace Sentinel {
    class Board {
        static inline std::map<char, int> pieceIndexMap = {{'p', 0},{'n', 1},{'b', 2},
                                                           {'r', 3},{'q', 4},{'k', 5}};

        static inline std::map<int, char> reversedPieceIndexMap = {{0, 'p'},{1, 'n'},{2, 'b'},
                                                                   {3, 'r'},{4, 'q'},{5, 'k'}};

        static inline std::map<char, int> files = {{'a', 0},{'b', 1},{'c', 2},{'d', 3},
                                                   {'e', 4},{'f', 5},{'g', 6},{'h', 7}};

        static inline std::map<char, int> ranks = {{'1', 7},{'2', 6},{'3', 5},{'4', 4},
                                                   {'5', 3},{'6', 2},{'7', 1},{'8', 0}};
    ...
    public:
        ...
        inline int getPieceType(int square) const {
            for (int j = 0; j < 6; j++) {
                if (bit_ops::getNthBit(whitePieces[j], square) || bit_ops::getNthBit(blackPieces[j], square))
                    return j;
            }
            return NO_PIECE;
        }
    ...
    private:
        ...
        inline void handleEnemyCastling(const std::pair<uint64_t, bool> &type, const Move &move, bool whoPlay,
                                        bool *enemyCastling) {
            switch (type.first) {
                case ROOK:
                    if (whoPlay) {
