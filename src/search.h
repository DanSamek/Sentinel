...
namespace Sentinel{
    ...
    public:
        ...
        std::pair<Move, uint64_t> findBestMove(int timeRemaining, int increment, Board& board,
                                               bool exact, int maxDepth, bool inf){
            prepareForSearch();

            _board = &board;
            _forceStopped = false;

            int msCanBeUsed = Timemanager::getSearchTime(timeRemaining, increment, exact);
            ...
            for (int depth = 1; depth < maxDepth; depth++){
                ...
                if(depth <= 5){
                    score = negamax<PV>(depth, 0, alpha, beta, true);
                ...
                int reduction = 0;
                while (!_forceStopped){
                    score = negamax<PV>(depth - reduction, 0, alpha, beta, true);
    ...
    private:
        ...
        int negamax(int depth, int ply, int alpha, int beta, bool doNull){
    #if !RUN_DATAGEN
            ...
            const bool isSingular = ss[ply].excludedMove != NO_MOVE;

            const TranspositionTable::Entry* entry = &TranspositionTable::NO_ENTRY;
            int ttIndex =  TT->index(_board->zobristKey);
            int ttEval = TranspositionTable::LOOKUP_ERROR;
            ...
            // IIR
            if (ttEval == TranspositionTable::LOOKUP_ERROR && ply > 0 && depth >= IIR_DEPTH) depth--;
            ...
            int alphaMd = std::max(alpha, -CHECKMATE + ply);
            int betaMd = std::min(beta, CHECKMATE - ply - 1);
            if (ply > 0 && alphaMd >= betaMd){
                return alphaMd;
            }

            // If current king is checked, logically we can't do NMP (enemy will capture our king).
            bool isCheckNMP = _board->inCheck();
            ...
            // Improving heuristic
            const bool improving = ply >= 2 && currentEval > ss[ply - 2].score + IMPROVING_CONSTANT && !isCheckNMP;
            ...
            if (!isSingular){
                ...
                if (!isPv && depth >= NMP_DEPTH && doNull && !isCheckNMP && someBigPiece && ply > 0){
                    _board->makeNullMove();
                    ss[ply].move = NO_MOVE;

                    int R = NMP_REDUCTION_CONSTANT + depth / NMP_REDUCTION_DIV_CONSTANT;
                    int eval = -negamax<NON_PV>(depth - R + 1, ply + 1, -beta, -beta + 1, false);
                    _board->undoNullMove();
                ...
                if (!isPv && !isCheckNMP && ply > 0 && depth <= RFP_DEPTH && currentEval - RFP_CONSTANT * depth >= beta){
                    return currentEval;
                }
            }
            ...
            // "move ordering"
            Move prevMove = ply == 0 ? NO_MOVE : ss[ply - 1].move;
            Move counterMove = prevMove.fromSq == -1 ? NO_MOVE : hist.counterMoves[prevMove.fromSq][prevMove.toSq];
            ...
            const bool canSingular = !isSingular && ply > 0 && depth >= SI_DEPTH &&
                                     entry->flag != TranspositionTable::UPPER_BOUND && entry->depth + SI_DEPTH_TT_ADD >= depth
                                     && std::abs(ttEval) < CHECKMATE_LOWER_BOUND;

            const bool isTTCapture = entry->best.isCapture();
            for (int j = 0; j < moveCount; j++){
                ...
                if (!isCapture && ply > 0 && !isCheckNMP){
                    ...
                    // Late move pruning.
                    int lmpLimit = LMR_MOVE_COUNT_ADD + depth * depth;
                    if (!isPv && quietMovesCount > lmpLimit && depth <= LMP_DEPTH && !moves[j].isPromotion()){
                        continue;
                    }

                    // Futility pruning.
                    if (!isPv && !isSingular && movesSearched > 0 &&
                        depth <= FP_DEPTH && currentEval + FP_CONSTANT * depth <= alpha){
                        continue;
                    }

                    // SEE pruning of quiet moves.
                    if (depth <= SEE_QUIET_DEPTH && alpha > -CHECKMATE
                        && !_board->SEE(moves[j], -SEE_QUIET_THRESHOLD*depth)){
                        continue;
                    }
                }

                // SEE pruning of captures.
                else if (ply > 0 && depth <= SEE_CAP_DEPTH && isCapture
                         && !_board->SEE(moves[j], -SEE_CAP_THRESHOLD*depth*depth)){
                    continue;
                }
                ...
                if (canSingular && moves[j] == entry->best){
                    const int singularBeta = ttEval - depth * SI_DEPTH_MUL;
                    const int singularDepth = (depth - 1) / 2;

                    ss[ply].excludedMove = entry->best;
                    const int singularEval = negamax<NON_PV>(singularDepth, ply, singularBeta - 1, singularBeta, true);
                ...
                if (movesSearched == 0 && isPv){
                    eval = -negamax<PV>(newDepth - 1, ply + 1, -beta, -alpha, true);
                }
                else{
                    // do reduced search (null window)
                    eval = -negamax<NON_PV>(newDepth - 1 - R, ply + 1, -alpha - 1, -alpha, true);

                    // do non reduced search (null window)
                    if (eval > alpha && R != 0) eval = -negamax<NON_PV>(newDepth - 1, ply + 1, -alpha - 1, -alpha, true);

                    // if LMR fails, do normal full search.
                    if (eval > alpha && eval < beta) eval = -negamax<PV>(newDepth - 1, ply + 1, -beta, -alpha, true);
                }
        ...
        int qsearch(int alpha, int beta, int ply){
            ...
            int ttIndex =  TT->index(_board->zobristKey);
            int ttEval = TT->getEval(_board->zobristKey, ttIndex, 0, alpha, beta, ply);
            Move hashMove = ttEval == TranspositionTable::FOUND_NOT_ACCEPTED ? TT->entries[ttIndex].best : NO_MOVE;
            ...
            for(int j = 0; j < moveCount; j++){
                ...
                // pseudolegal movegen.
                if (!_board->makeMove(moves[j])) continue;
        ...
        void printInfo(int depth, Timer idTimer){
            double ms = idTimer.getMs() / 1000.0;
            int nps = ss.nodesVisited / (ms == 0 ? 1 : ms);
